...
@if (!empty($video))
    <section class="video wow animate__animated animate__fadeInUp" style="{{(!empty($padding)) ? 'padding:'.$padding.'px 0px;' : ''}} {{(!empty($videomarco)) ? "background-image: url('images/".$videomarco."');" : ''}} background-repeat:repeat-x;">
        @empty(!$videoantetitulo)
            <span>{{$videoantetitulo}}</span>
        @endempty
        @empty(!$videotitulo)
            <h2>{{$videotitulo}}</h2>
        @endempty
        <div class="videoPlayer {{$videoformato}} wow animate__animated animate__fadeInUp" data-wow-delay="0.6s">
            @if($videocanal == "youtube")
            <div class="player" style="background-color:none;">
                <iframe src="https://www.youtube-nocookie.com/embed/{{$video}}?si=7CQt0gnEV97CNWwW&amp;controls=0" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen  style="border-radius:8px;"></iframe>
            </div>
            @endif
            @if ($videocanal == "vimeo")
            <div class="player">
                <iframe src="https://player.vimeo.com/video/{{$video}}?h=b86574a207&autoplay=1&color=A2AE8C&title=0&byline=0&portrait=0" width="640" height="1138" frameborder="0" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen  style="border-radius:8px;"></iframe>
            </div>
            @endif
        </div>
    </section>
