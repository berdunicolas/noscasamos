...
<section class="content full" style="background-image:url('{{$module['image']}}');">
    <div class="info">
        <div class="text wow animate__animated animate__fadeInUp">
            @empty(!$module['icon'])
                @if($icontype==='Animado')
                    <lord-icon src="https://cdn.lordicon.com/{{$module['icon']}}.json" trigger="{{$trigger}}" state="{{$destacadoicons}}" stroke="{{$stroke}}" delay="500" colors="primary:#fff,secondary:{{$color}}" style="width:70px;height:70px"></lord-icon>
                @else
